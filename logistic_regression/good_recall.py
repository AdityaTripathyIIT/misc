...
class LogisticRegressionIRLS:
    ...
    def predict(self, X):
        ...
        return (y_pred > 0.6).astype(int)


def robust_standardise():
    global X_train, X_test
    medians = np.median(X_train, axis=0)
    iqr = np.percentile(X_train, 75, axis=0) - \
        np.percentile(X_train, 25, axis=0)
    iqr[iqr == 0] = 1
    X_train = (X_train - medians) / iqr
    X_test = (X_test - medians) / iqr
...
# Number of samples to add to match majority class
...
minority_oversampled = minority_class.sample(
    n=n_to_add, replace=True, random_state=42)

# Combine everything into a new balanced dataset
df_balanced = pd.concat(
    [majority_class, minority_class, minority_oversampled], axis=0)

# Shuffle the dataset
df_balanced = df_balanced.sample(
    frac=1.0, random_state=42).reset_index(drop=True)

X_train = df_balanced.drop(columns=['label']).values
...
print(f"Custom Logistic Regression (IRLS) Accuracy: {custom_acc_irls:.4f}")
print(f"Custom Logistic Regression (IRLS) Precision: {custom_prec_irls:.4f}")
...
print(f"Sklearn Logistic Regression Precision: {sklearn_prec:.4f}")
